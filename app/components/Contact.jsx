...
const inputCls =
  "w-full border border-white/10 rounded-lg px-3 sm:px-3.5 py-2 sm:py-2.5 text-xs sm:text-sm text-white bg-white/5 outline-none transition-all duration-200 placeholder:text-white/20 focus:border-[#2de8b0] focus:ring-2 focus:ring-[#2de8b0]/10 focus:bg-white/10";
...
const Checkbox = ({ label, checked, onToggle }) => (
  <label className="flex items-center gap-2 cursor-pointer select-none group">
    ...
    <span className="text-sm lg:text-base text-white/50 group-hover:text-white/80 transition-colors duration-150 font-medium">
      {label}
    </span>
  </label>
);
...
const Field = ({ label, children, htmlFor }) => (
  <div className="flex flex-col gap-1">
    <label
      htmlFor={htmlFor}
      className="text-xs lg:text-sm font-semibold text-white/40 tracking-wide uppercase"
    >
      {label}
    </label>
    {children}
...
const ContactForm = () => {
  ...
  return (
    ...
    >
      ...
      <fieldset>
        <legend className="text-xs lg:text-sm font-semibold text-white/40 tracking-wide uppercase mb-2">
          Services
        </legend>
      ...
      {status && (
        <p className="text-xs sm:text-sm mt-2 text-center text-[#2de8b0]">
          {status}
        </p>
      )}
    </form>
  ...
  return (
    <>
      <section
        id="contact"
        ref={containerRef}
        className={`relative w-full min-h-screen flex items-center justify-center p-4 sm:p-6 lg:p-8 ${urbanist.className}`}
        style={{
      ...
      >
        <div
          ref={cardRef}
          className="flex flex-col md:flex-row w-full max-w-7xl min-h-screen md:min-h-auto md:rounded-3xl overflow-hidden backdrop-blur-3xl bg-white/5 border border-white/10 shadow-2xl"
          style={{ opacity: 0 }}
        >
          {/* Left Visual Side */}
          <div className="relative w-full h-48 sm:h-56 md:h-auto md:w-[42%] shrink-0 overflow-hidden">
            ...
            <div className="absolute inset-0 bg-linear-to-t from-black via-black/20 to-transparent p-8 flex flex-col justify-end">
              <p className="text-[#2de8b0] text-xs font-medium tracking-widest uppercase mb-1">
                Start a project
              </p>

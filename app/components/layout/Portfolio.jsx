...
const projects = [
    ...
    {
        id: 14,
        title: "Language Learning Landing Page",
        subtitle: " Landing Page",
        category: "Business",
        platform: "WordPress",
        thumbnail: "/images/our-work/langl.png",
        liveUrl: "https://darkseagreen-ferret-910390.hostingersite.com/",
        tags: ["WordPress", "Elementor", "Astra"],
        accent: "#0891b2",
        accentBg: "rgba(8,145,178,0.09)",
    },
...
/* On mobile (1 col) all cards slide from bottom.
   On tablet (2 col): col0 → left, col1 → right.
   On desktop (3 col): col0 → left, col1 → bottom, col2 → right. */
const getSlideOrigin = (colIndex, isMobile, isTablet) => {
    if (isMobile) return { x: 0, y: 50 };
    if (isTablet) return colIndex % 2 === 0 ? { x: -70, y: 0 } : { x: 70, y: 0 };
    if (colIndex === 0) return { x: -90, y: 0 };
    if (colIndex === 2) return { x: 90, y: 0 };
    return { x: 0, y: 60 };
};

/* ─── CARD ─── */
const ProjectCard = ({ project, colIndex }) => {
    ...
    useEffect(() => {
        ...
        // Detect breakpoint at mount time
        const w = window.innerWidth;
        const isMobile = w < 640;
        const isTablet = w >= 640 && w < 1024;
        const { x, y } = getSlideOrigin(colIndex, isMobile, isTablet);

        gsap.set(el, { x, y, opacity: 0, scale: 0.94 });
    ...
    return (
        <div
            ...
            style={{
                ...
                boxShadow: hovered
                    ? "0 20px 56px rgba(0,0,0,0.11), 0 6px 16px rgba(0,0,0,0.06)"
        ...
        >
            ...
            <div className="flex flex-col flex-1 px-4 pt-3.5 pb-4 sm:px-5 sm:pt-4 sm:pb-5">
                ...
                >
                    <span
                        className="font-semibold uppercase text-gray-300"
                    ...
                    <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="flex items-center justify-center w-7 h-7 rounded-full no-underline"
                        style={{
...
const Portfolio = () => {
    ...
    const headerSTsRef = useRef([]);

    const filtered = projects.filter((p) =>
        activeFilter === "All" ? true : p.platform === activeFilter || p.category === activeFilter
    );

    /* Header + filter scrub animations */
    useEffect(() => {
        headerSTsRef.current.forEach((t) => t.kill());
        headerSTsRef.current = [];

        if (headingRef.current) {
            headingRef.current.querySelectorAll(".anim-el").forEach((el) => {
                ...
                const t = ScrollTrigger.create({
                    ...
                    scrub: 1.2,
                    onUpdate: (self) => {
                        gsap.set(el, { y: 28 * (1 - self.progress), opacity: self.progress });
                    },
                });
                headerSTsRef.current.push(t);
            });
        }

        if (filterBarRef.current) {
            filterBarRef.current.querySelectorAll(".filter-pill").forEach((pill) => {
                gsap.set(pill, { y: 16, opacity: 0 });
                const t = ScrollTrigger.create({
                    trigger: pill,
                    start: "top 94%",
                    end: "top 72%",
                    scrub: 1,
                    onUpdate: (self) => {
                        gsap.set(pill, { y: 16 * (1 - self.progress), opacity: self.progress });
                    },
                });
                headerSTsRef.current.push(t);
            });
        ...
        return () => headerSTsRef.current.forEach((t) => t.kill());
    }, []);

    return (
        <section id="portfolio" className={`w-full py-16 sm:py-24 lg:py-32  ${urbanist.className}`}>
            <div className="w-full max-w-350 mx-auto px-4 sm:px-6 lg:px-10">
                ...
                <div ref={headingRef} className="mb-10 sm:mb-12">
                    ...
                    <div className="anim-el flex items-center gap-2 mb-4 sm:mb-5">
                        ...
                        <span
                            className="text-[#23bcdf] font-semibold uppercase tracking-[0.18em]"
                    ...
                    <div className="anim-el flex flex-col gap-3 mb-2 md:flex-row md:items-end md:justify-between md:gap-6">
                        ...
                        <p
                            className="text-gray-400 leading-relaxed md:max-w-65 lg:max-w-75"
                    ...
                    {/* Divider */}
                    <div className="anim-el mt-6 h-px bg-gray-200" />
                </div>
